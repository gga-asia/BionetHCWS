    ...
    {       
        ...
        public string BionetWEBUH001(string USER_ID, string USER_PW, string AUTH_TOKEN,string JSONSTR)
        {
            ...
            if (Name == "")
            {
                ...
                File.AppendAllText(pathLogFile, "\r\n" + TodyMillisecond + " 錯誤訊息：" + strstatus);
            }
            //1.2如果有，接下去，解密JSON    
            //2.讀取JSON，做判別，是否寫入
            else
...
//            string AAA = EncryptString(JSONSTR, "0601160016"); 
...
//            string AAA = DecryptString(JSONSTR, "3512012595");     //送子鳥 將回傳string解密
            ...
            if (strstatus == "")
            {
                ...
                if (HOS_INVOICE != "")
                {
                    ...
                    STRJSON = DecryptString(JSONSTR, HOS_INVOICE);
...
//                    string strAUTH_TOKEN = Decrypt(antiData(AUTH_TOKEN));
        ...
        public string step1(string jsonStr, string USER_ID, string AUTH_TOKEN)
        {
            try
            {
                ...
                Insert_HcJsondata(ExlInfoNo, jsonStr, CUS_ID, HIS_TITLE, CUS_NAME, GET_BLOOD_D, CUS_BIRTH
...
//                            HttpContext.Current.Response.Write(sw.ToString());
            ...
            catch (Exception ex)
            {
                ...
                File.AppendAllText(pathLogFile, "\r\n" + TodyMillisecond + " 錯誤訊息：9999--" + ex.Message);
...
//                string path = @"D:\out\" + folder + @"\" + cusCtId + ".pdf";
        ...
        public void UpdateFlag(string CusID,string ProductType)
        {
            ...
            //寫LOG
            //把內容寫到目的檔案，若檔案存在則附加在原本內容之後(換行)
            File.AppendAllText(pathLogFile, "\r\n" + TodyMillisecond + "將身分證號：" + CusID + "的Report_Upload=Y ");
        }
        ...
        public string GetHospitalData(string USER_ID, string USER_PW, string AUTH_TOKEN)
        {
            ...
            //寫LOG
            //把內容寫到目的檔案，若檔案存在則附加在原本內容之後(換行)
            File.AppendAllText(pathLogFile, "\r\n" + TodyMillisecond + " 查出是否有這個合作醫院(HOSPITAL_COOPERATION) SQL語法:" + sql);
...
//                sql += " AND A.PRINTDATE > '" + DateTime.Now.AddMonths(-1).ToString("yyyy/MM/dd") + "'";
        ...
        public string GetProductList(string strHOS_ID)
        {
            ...
            //寫LOG
            //把內容寫到目的檔案，若檔案存在則附加在原本內容之後(換行)
            File.AppendAllText(pathLogFile, "\r\n" + TodyMillisecond + "  取HC_EXLINFO中所有資料 SQL語法:" + sql);
        ...
        public void InsertData(string ExlInfoNo,string jsonStr)
        {
            ...
            //寫LOG
            //把內容寫到目的檔案，若檔案存在則附加在原本內容之後(換行)
            File.AppendAllText(pathLogFile, "\r\n" + TodyMillisecond + "  寫入HC_JSONDATA SQL語法:" + cmd.CommandText);
        }

        //寫入HC_JSONDATA (2018.02.13.新增身分證號及Flag狀態)
        public void Insert_HcJsondata(string ExlInfoNo, string jsonStr, string CusID, string ProductType, string CUS_NAME, string GET_BLOOD_D
                                    ...
                                    , string USER_ADDR, string USER_UB_DATE, string HOS_ID)
        {
            ...
            try
            {
                ...
                cmd.CommandText += ";SELECT IDENT_CURRENT ('HC_JSONDATA') AS SN";
                ...
                int SN = Convert.ToInt16(cmd.ExecuteScalar());


                //(2)記錄該人
                cmd.CommandText = "INSERT INTO HC_CUS_DATA(SN, ExlInfoNo, Cus_Name, Cus_CID, GetBlood_D, Cus_Birth" +
                                  ", Patient_ID, Memo_Teacher, EMail, Moible, Cus_Sex, Addr, UB_DATE, HOS_ID, CreateDate) " +
                                  "VALUES (@SN, @ExlInfoNo, @Cus_Name, @Cus_CID, @GetBlood_D, @Cus_Birth " +
                                  ",@Patient_ID, @Memo_Teacher, @EMail, @Moible, @Cus_Sex, @Addr, @UB_Date, @HOS_ID, @CreateDate, @ProductType)";
            ...
            if (errtxt == "")
            {
                ...
                File.AppendAllText(pathLogFile, "\r\n" + TodyMillisecond + "  寫入HC_JSONDATA SQL語法:" + cmd.CommandText + "(" + p.Substring(1) + ")");
            }
            else
                File.AppendAllText(pathLogFile, "\r\n" + errtxt);
